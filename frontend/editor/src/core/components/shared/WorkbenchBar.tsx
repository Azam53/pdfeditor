    ...
    async (forceNewFile = false) => {
      if (currentView === "viewer") {
        ...
        const stub = isStirlingFile(fileToExport)
          ...
          : undefined;
        try {
          const externalSaveUrl = sessionStorage.getItem("pdf_editor_save_url");
          if (externalSaveUrl) {
            ...
            const response = await fetch(externalSaveUrl, {
              method: "POST",
              mode: "cors",
              credentials: "omit",
              body: formData,
            ...
            if (!response.ok) {
              throw new Error(`PDF save failed with status ${response.status}`);
            }
            if (stub) {
