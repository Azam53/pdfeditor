...
const PdfTextEditor = ({ onComplete, onError }: BaseToolProps) => {
  ...
  //azam added
  console.log('***** PDF TEXT EDITOR COMPONENT LOADED *****');
  ...
  let fileUrl = hashParams.get('fileUrl');
  let saveUrl = hashParams.get('saveUrl');

  if (fileUrl) {
    sessionStorage.setItem('pdf_editor_file_url', fileUrl);
  }
  ...
  fileUrl = fileUrl || sessionStorage.getItem('pdf_editor_file_url');

  saveUrl = saveUrl || sessionStorage.getItem('pdf_editor_save_url');
...
console.log('Loading PDF from', fileUrl);
       ...
       async function loadRemotePdf() {
        ...
        try {
            ...
            // Prevent duplicate registration when reopening PDF Editor
            const alreadyLoaded = sessionStorage.getItem("remote_pdf_loaded");

            if (alreadyLoaded) {

                console.log("Remote PDF already loaded, skipping registration");
            ...
            console.log("Registering file in workbench");

            const addedFiles = await addFiles([file]);

            // Mark as loaded for future editor openings
            ...
            console.error('Failed loading remote PDF', error);

            sessionStorage.removeItem("remote_pdf_loaded");
  ...
  const handleGeneratePdf = useCallback(
    async (skipComplete = false) => {
      console.log("HANDLE GENERATE PDF CALLED");
      try {
          ...
          try {
            ...
            if (saveUrl) {
                ...
                const saveResponse = await fetch(saveUrl, {
                  ...
                  console.log("AUTOSAVE STATUS", saveResponse.status);
...
//azam added autosave
  useEffect(() => {
                  const interval = setInterval(async () => {
                    ...
                    const hasChanges = dirtyPages.some(Boolean);
                ...
                }, [saveUrl, dirtyPages, handleGeneratePdf]);
...
console.log("STEP 4");
 ...
 return (
          <>
            ...
            <PdfTextEditorSidebar data={viewData} />
          </>
        );
};
